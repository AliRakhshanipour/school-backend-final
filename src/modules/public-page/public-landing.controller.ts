// src/modules/public-page/public-landing.controller.ts
...
export class PublicLandingController {
  ...
  @ApiOperation({
    summary:
      'لیست بخش‌های فعال صفحه عمومی (برای لندینگ)',
  })
  listSections() {
  ...
  @ApiOperation({
    summary:
      'دریافت یک بخش فعال صفحه عمومی بر اساس sectionKey',
  })
  getSection(@Param('key') key: string) {
    return this.publicPageService.publicGetSectionByKey(
      key,
    );
  }
  ...
  @ApiOperation({
    summary:
      'دیتای کامل لندینگ: سکشن‌ها + چند خبر عمومی + وضعیت پیش‌ثبت‌نام',
  })
  landing() {
