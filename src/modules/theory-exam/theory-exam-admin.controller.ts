...
import { CreateTheoryExamByAdminDto } from './dto/create-theory-exam-by-admin.dto';
...
export class TheoryExamAdminController {
  ...
  @ApiOperation({
    ...
    description: 'courseAssignmentId باید از قبل ایجاد شده باشد و درس آن تئوری باشد.',
  })
  createByAdmin(@Body() dto: CreateTheoryExamByAdminDto) {
    return this.theoryExamService.createExamByAdmin(dto.courseAssignmentId, dto);
  }

  @Get()
  @ApiOperation({
    summary: 'لیست امتحان‌های تئوری با فیلترهای اختیاری (سال، کلاس، نوبت، انتساب)',
  })
  ...
  list(
    ...
    @Query('term', new ParseEnumPipe(ExamTerm, { optional: true })) term?: ExamTerm,
  ...
  @Get(':examId')
  @ApiOperation({ summary: 'جزئیات کامل یک امتحان (به همراه نمرات) برای مدیر/معاون' })
  getDetails(@Param('examId', ParseIntPipe) examId: number) {
    return this.theoryExamService.adminGetExamDetails(examId);
  }

  @Patch(':examId')
  @ApiOperation({ summary: 'ویرایش اطلاعات یک امتحان توسط مدیر (زمان، عنوان، وزن، ...)' })
  update(
  ...
  @Patch(':examId/lock')
  @ApiOperation({ summary: 'قفل کردن یک امتحان (بعد از نهایی شدن نمرات، دیگر قابل تغییر نیست)' })
